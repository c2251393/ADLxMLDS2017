...
class BRESR(nn.Module):
    def __init__(self,
        ...
        self.lstm = nn.LSTM(input_size,
                            ...
                            bidirectional=True)
        self.decoderlr = nn.Linear(self.hidden_size, self.hidden_size)
        self.decoderrl = nn.Linear(self.hidden_size, self.hidden_size)
    ...
    def forward(self, input, hc, lens):
        ...
        # output: (batch x maxlen x 2H)

        output = self.decoderlr(output[:,:,:self.hidden_size]) + self.decoderrl(output[:,:,self.hidden_size:])
        # output: (batch x maxlen x H)

        output = output.unsqueeze(1)
        # output: (batch x 1 x maxlen x H)
        output = self.res1(output)
        # output: (batch x 16 x maxlen x H)
        output = self.res2(output)
        # output: (batch x 8 x maxlen x H)
        output = self.res3(output)
        # output: (batch x 4 x maxlen x H)
        output = self.res4(output)
        # output: (batch x 2 x maxlen x H)
